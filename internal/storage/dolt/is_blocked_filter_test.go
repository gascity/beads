...
// TestIsBlockedFilterPlanIsIndexed is the sargability guard for the §13.7
// IssueFilter.IsBlocked predicate: `is_blocked = ?` must seek
// idx_issues_is_blocked(is_blocked, status) (IndexedTableAccess), not
// full-scan-and-filter, so the board's (status × is_blocked) matrix stays cheap
...
func TestIsBlockedFilterPlanIsIndexed(t *testing.T) {
	...
	// a literal so the planner shape is under test (the board binds 1 and 0).
