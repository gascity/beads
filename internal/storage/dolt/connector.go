...
//
// go-sql-driver/mysql (v1.9+) clones the Config before invoking BeforeConnect
...
// resolveDialToken resolves a live gateway token for a new physical connection
// dial. A package var so tests can stub the resolution without a live helper; the
// default (resolveGatewayDialToken, gateway_credential.go) applies the same
// fail-closed validations as the open-time ApplyGatewayCredential.
var resolveDialToken = resolveGatewayDialToken
...
//
// The creds command cache holds tokens with a 10s pre-expiry skew, so the common
// case is a mutex-guarded map hit (~µs); only near the token expiry boundary does
// it shell out to the helper. On any resolution error the hook fails closed — the
// dial is aborted rather than falling back to a stale token. It is a named
// function so tests can invoke it without a live server.
func credentialBeforeConnect(credCmd string) func(context.Context, *mysql.Config) error {
	return func(ctx context.Context, c *mysql.Config) error {
		tok, err := resolveDialToken(ctx, credCmd)
		if err != nil {
