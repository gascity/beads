...
type EventQueryStore interface {
	...
	// row's cursor must therefore re-scan a slack window behind it (the API
	// consumer contract mandates a 45s overlap) and de-duplicate by id, or it
	// can skip an event whose created_at preceded the cursor but which committed
