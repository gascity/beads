...
// bare indexed column.
//
// On Postgres — the hosted API backend — this plans as a BitmapOr index-merge
// over idx_dep_{issue,wisp,external}_target (a Bitmap Heap Scan, cost ~36 on a
// 5k-row table) where the COALESCE form is a Seq Scan (cost ~104). On Dolt
