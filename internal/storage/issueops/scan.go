...
// IssueSelectColumnsLite is the column list for lite issue hydration. It
// mirrors IssueSelectColumns in order, minus the heavy text columns enumerated
// in HeavyDropList. Callers opt in with types.IssueFilter.Lite.
//
// Metadata and lease timestamps are retained because they are small fields
// used by routing and claim code, rather than the large issue bodies this
// projection exists to skip.
...
// The schema-parity test uses it to require every full-projection column to be
// classified as either retained or deferred. Production code selects one of
// the two explicit column lists and does not consume HeavyDropList at runtime.
...
// ScanIssueLiteFrom scans a partially hydrated issue from a query selecting
// IssueSelectColumnsLite in exactly that order. The omitted heavy fields stay
// zero-valued, and IsLitePartial marks that they were not fetched.
func ScanIssueLiteFrom(s IssueScanner, extra ...any) (*types.Issue, error) {
	var issue types.Issue
	var createdAtStr, updatedAtStr sql.NullString
	var startedAt, closedAt, compactedAt, dueAt, deferUntil sql.NullTime
	var leaseExpiresAt, heartbeatAt sql.NullTime
	var estimatedMinutes, originalSize, timeoutNs sql.NullInt64
	var createdBy sql.NullString
	var assignee, externalRef, specID, compactedAtCommit, owner sql.NullString
	var contentHash, sourceRepo, closeReason sql.NullString
